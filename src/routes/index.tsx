      ...
      <div className="mx-auto max-w-7xl px-6 pt-20 pb-24 md:pt-28 md:pb-32">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <Link to="/pricing" className="inline-flex w-full items-center justify-center rounded-lg border border-border bg-surface px-7 py-3.5 text-base font-semibold text-foreground transition hover:bg-bg-secondary sm:w-auto">
              شاهد الباقات
            </Link>
          </div>
        ...
        <div className="mx-auto mt-16 grid max-w-4xl grid-cols-3 gap-4">
          {[
            ...
            <div key={c.en} className="rounded-2xl border border-border bg-surface p-6 text-center shadow-premium">
              <div className="font-mono text-xs uppercase tracking-widest text-text-tertiary">{c.en}</div>
              <div className="mt-2 text-2xl font-black text-foreground">{c.ar}</div>
              <div className="mt-1 text-xs text-text-secondary">متوسط القيمة <span className="text-gold font-mono font-bold">{c.val} ر.س</span></div>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="features" className="py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-black text-foreground md:text-4xl">حماية تليق بـ <span className="text-gold">قيمة</span> أصلك</h2>
          <p className="mt-4 text-text-secondary">مزجنا التغطية التأمينية الكلاسيكية بتقنيات الذكاء الاصطناعي ومراقبة المستشعرات لنقدم تجربة لم تكن موجودة من قبل.</p>
        </div>
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {items.map((it) => (
            <div key={it.title} className="group rounded-2xl border border-border bg-surface p-7 shadow-premium transition hover:-translate-y-1 hover:shadow-gold">
    ...
    <section id="how" className="bg-bg-secondary py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-black text-foreground md:text-4xl">من التسجيل إلى الحماية في <span className="text-teal">دقيقة</span></h2>
        </div>
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {steps.map((s) => (
            <div key={s.n} className="relative rounded-2xl border border-border bg-surface p-8 shadow-premium">
    ...
    <section className="py-24">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 md:grid-cols-3">
        {[
          ...
          <figure key={t.a} className="rounded-2xl border border-border bg-surface p-7 shadow-premium">
